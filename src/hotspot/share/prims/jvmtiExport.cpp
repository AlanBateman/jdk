...
// Returns true if a field access or modification event should be skipped while the event
// thread is initializing a class with strictly-initialized static fields or constructing
// an object with strictly-initialized fields in its class hierarchy.
static bool should_skip_field_event_during_strict_initialization(JavaThread* thread, Klass* field_klass, Handle object, jfieldID field) {
  ...
  if (!JvmtiEnv::get_field_descriptor(field_klass, field, &fd)) {
    assert(false, "should_skip_field_event_during_strict_initialization called with invalid fieldID");
    return true;
  }

  // Skip a field event for a static field when the class declaring the field is being
  // initialized by the event thread and the class declares strictly-initialized static fields.
  if (fd.is_static()) {
    InstanceKlass* ik = fd.field_holder();
    return ik->has_strict_static_fields() && ik->is_reentrant_initialization(thread);
  }

  // Skip a field event for an instance field when the object containing the field is being
  // constructed by the event thread and the object's class, or any of its superclasses,
  // has strictly-initialized instance fields.
  ResourceMark rm(thread);
  RegisterMap reg_map(thread,
                      RegisterMap::UpdateMap::skip,
                      RegisterMap::ProcessFrames::skip,
